...
// 
//  HDF5 Loop Writing without derivatives
//


//-C.K: Funtion to write the loops in HDF5 format (Standard Momenta Form)
template<typename Float>
void writeLoops_HDF5_NoDer_StrdMomForm(Float *buf_std_uloc, Float *buf_gen_uloc, 
				 char *file_pref, 
				 qudaQKXTM_loopInfo loopInfo,
				 bool exact_loop){
...
//-C.K: Funtion to write the loops in HDF5 format (High Momenta Form)
template<typename Float>
void writeLoops_HDF5_NoDer_HighMomForm(Float *buf_std_uloc, Float *buf_gen_uloc,
                                 char *file_pref,
                                 qudaQKXTM_loopInfo loopInfo,
                                 bool exact_loop){
  ...
  if(GK_timeRank==0){
    ...
    char *cNmoms, *cQsq,*loop_info,*ens_info;
    char *loop_str1,*loop_str2,*loop_str3,*loop_str4,*loop_types[2];
    asprintf(&loop_types[0],"Scalar  - Ultra-local    operators, Standard    one-end trick\n");
    asprintf(&loop_types[1],"dOp     - Ultra-local    operators, Generalized one-end trick\n");
    asprintf(&loop_str1,"Momentum-space quark loop\nQuark field basis: Twisted\n");
    asprintf(&loop_str2,"Precision: %s\n",(typeid(Float) == typeid(float)) ? "single" : "double");
    asprintf(&loop_str3,"Inversion tolerance: %e\n", loopInfo.inv_tol);
    asprintf(&loop_str4,"Loop types:\n  %s  %s\n",loop_types[0],loop_types[1]);
...
template<typename Float>
void writeLoops_HDF5_NoDer(Float *buf_std_uloc, Float *buf_gen_uloc,
                     char *file_pref,
                     qudaQKXTM_loopInfo loopInfo,
                     bool exact_loop){

  if(loopInfo.HighMomForm)
    writeLoops_HDF5_NoDer_HighMomForm<double>(buf_std_uloc, buf_gen_uloc,
                                        file_pref, loopInfo,
                                        exact_loop);
  else
    writeLoops_HDF5_NoDer_StrdMomForm<double>(buf_std_uloc, buf_gen_uloc,
                                        file_pref, loopInfo,
                                        exact_loop);
